"""Synchronous Boomclaw SDK client using ``requests``."""
...
class BoomclawSDK:
    """Synchronous client for the Boomclaw External Game SDK.
    ...
    def get_session(self, token: str) -> SessionInfo:
        ...
        data = self._request("GET", "/api/sdk/session", token)
    ...
    def charge(
        ...
        data = self._request("POST", "/api/sdk/charge", token, json_body={
            "amount": amount,
            "item_key": item_key,
    ...
    def save(self, token: str, key: str, value: dict) -> dict:
        """Save game state to the platform."""
        return self._request("POST", "/api/sdk/save", token, json_body={
            "key": key,
            "value": value,
        })

    def load(self, token: str, key: str) -> dict:
        """Load game state from the platform."""
        return self._request("GET", "/api/sdk/load", token, params={"key": key})

    def end_game(self, token: str, result: dict | None = None) -> dict:
        """Signal that the game session has ended."""
        return self._request("POST", "/api/sdk/end", token, json_body={
            "result": result,
        })
    ...
    def submit_score(
        ...
        data = self._request("POST", "/api/sdk/leaderboard/submit", token, json_body={
            "score": score,
            "metadata": metadata,
        })
    ...
    def get_leaderboard(
        ...
        data = self._request("GET", "/api/sdk/leaderboard", token, params={
            "period": period,
            "limit": limit,
            "offset": offset,
        })
    ...
    def register_achievements(
        ...
        return self._request("POST", "/api/sdk/achievement/register", token, json_body={
            "achievements": [asdict(a) for a in achievements],
        })

    def unlock_achievement(self, token: str, achievement_key: str) -> UnlockResult:
        """Unlock an achievement for the current player."""
        data = self._request("POST", "/api/sdk/achievement/unlock", token, json_body={
    ...
    def get_achievements(self, token: str) -> AchievementsResult:
        """Get all achievements and their status for the current player."""
        data = self._request("GET", "/api/sdk/achievements", token)
